...
remote string function CFSessionDisplayDataColumns (
	...
	// Special case for keyword, replace '+' with blank
	...
	try {
		...
		if (NOT IsArray(session.UserPreferences.aMyFilters)) {
			...
			for (var walker = 1; walker LE ArrayLen(session.UserPreferences.aMyFilters); walker += 1) {
				...
				// handle ' and & in filterName
				escFilterName = replaceNoCase(filterName, "'", "\'", "all");
...
<cffunction name="getContractListingHomeforCheckboxes" access="public" returntype="any" output="true" hint="Get listing of contracts for home page, USING CHECKBOXES">
	...
	<cfset arguments.status 		= buildQuotedValueList(arguments.status,"Alpha")>
	<cfset arguments.agreementType 	= buildQuotedValueList(arguments.agreementType,"Alpha")>
	<cfset arguments.fundingOffice	= buildQuotedValueList(arguments.fundingOffice,"Numeric")>
	<cfset arguments.serviceType	= buildQuotedValueList(arguments.serviceType,"Alpha")>
	<cfset arguments.searchWord		= trim(UCase(ReplaceNoCase(arguments.searchWord, '+', '_','all')))>
	<cfset arguments.State			= buildQuotedValueList(arguments.State,"Alpha")>
	...
	if (arguments.searchWord neq "") {
		...
		cmd1 = " ( UPPER(centername) LIKE '%#arguments.searchWord#%' OR UPPER(venue) LIKE '%#arguments.searchWord#%' OR UPPER(contractnum) LIKE '%#arguments.searchWord#%' OR UPPER(contractorname) LIKE '%#arguments.searchWord#%' OR UPPER(programActivity) LIKE '%#arguments.searchWord#%' )";

		if (local.aappNum NEQ 0) {
			// we already know there is a match on the aappNum
			cmd1 = " ( aappnum = #local.aappNum# OR " & cmd1 & " ) ";
		}
		...
		if ( sAAPPfromFootprint EQ '' ) {
			...
			// allow search in the document number
...
<!--- testing, and this BREAKS the Home AAPP list, for testing the timeout
<cfdump var="#cmd#">
<cfabort>
--->
...
public struct function CFSessionDisplayDataColumnsGuts (
	...
	// loop over the query to product the HTML for the table
	var sClass='';
	var sTRClass = '';
	if (textOnly) {
		sClass='form3DataTbl';
