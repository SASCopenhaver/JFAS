...
<form name="frmGraph" id="frmGraph">
    ...
    <div id="div_DataOPS">div_Data TEST area</div>
    <div id="div_DataCRA">div_Data TEST area</div>
    <div id="div_Graph"></div>
    
    
    <div id="div_GraphLegend">OPS</div>
    <br />
    <div id="div_GraphOPS"></div>
    <br />
    <div id="div_GraphCRA">CRA</div>
    <br />
...
function f_prepareData(arg_parsedJSON){
	...
	var objDatumOPS = objDatum.spr_GroupedBarOPS;
	var objDatumCRA = objDatum.spr_GroupedBarCRA;
	//$("#div_Graph").html("Parsed Data:\n"+JSON.stringify(objDatumCRA) );
	
	f_drawDatumOPSinD3(f_makeWorkableJSON(objDatumOPS), "#div_GraphOPS");
	f_drawDatumOPSinD3(f_makeWorkableJSON(objDatumCRA), "#div_GraphCRA");
	
}	// end of f_prepareData(arg_parsedJSON)
//--------------------------------------------------------------------------------------------
function f_drawDatumOPSinD3(arg_objDatum, arg_Div){
//
data = arg_objDatum;
...
if (arg_Div == "#div_GraphOPS"){var color = d3.scale.category20c();}
if (arg_Div == "#div_GraphCRA"){var color = d3.scale.category20b();}
...
var seriesNames = d3.keys(data[0]).filter(function (key) 
{
  ...
  var years = svg.selectAll(".years")
    ...
    .enter().append("g")
	  ...
	  //.on("mouseover", function(d){ alert( x0(d.YEARS) );} )
	  ;
  ...
  years.selectAll("rect")
	...
	.enter().append("rect")
	  ...
	  //.on("mouseover", function(d){ return color("red");} )
	  //.on("mouseout", function(d){ return color(d.name); })
	  ;
