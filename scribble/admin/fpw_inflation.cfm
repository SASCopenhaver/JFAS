...
<!--- add i to firstdate to get the start date --->
	<tr <cfif rowYear MOD 2> class="AltRow"</cfif>><!--- alternate row shading based on CalYear --->
		...
		<td align="center"><label for="id#rowYear#StartDate" class="hiddenLabel">Start Date</label>
			<input type="text" name="txt_#rowYear#_startDate" id="id#rowYear#StartDate" size="12" maxlength="10" value="#dateFormat(form['txt_' & rowYear & '_startDate'], "mm/dd/yyyy")#" <cfif form['txt_' & rowYear & '_status'] is 1>readonly class="inputReadonly"</cfif> onBlur="validDate('#rowYear#', this.value);" tabindex="#request.nextTabIndex#" />
			<cfset request.nextTabIndex = request.nextTabIndex + 1>
			<a href="javascript:MS_ShowFieldPopCalendar(frmFpwInflation['txt_' + #rowYear# + '_startDate'], 'M/D/Y')">
			<cfif form['txt_' & rowYear & '_status'] eq 0>
				<img src="#application.paths.images#calendar_icon.gif" border="0" name="img_#rowYear#_calendar" alt="Select to specify effective date" width="16" height="13" />
			<cfelse>
				<img src="#application.paths.images#clear.gif" width="1" name="img_#rowYear#_calendar" border="0" height="1" alt="">
			</cfif>
			</a>
		</td>
