...
<cffunction name="DisplayHTMLSetup" hint="I am a function to display the top-most banner for the jfas home page.">
	...
	<!--- display one-time alert message upon login --->
	<!--- TODO: build data driven alert module --->
	<cfif session.ncfmsAlertDisplay>
		<script>
		ncfmsAlert = 'JFAS SYSTEM UPDATE: NCFMS Obligational Data are now up to date and available in JFAS.\n\n' +
					  'Inclusion of NCFMS transaction level data is still in development.\n\n' +
					  'Further notices will be sent out when this is available.';
		alert(ncfmsAlert);
		</script>
		<cfset session.ncfmsAlertDisplay = 0>
	</cfif>
