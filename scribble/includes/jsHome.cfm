...
glJFAS.sLogoutNoMessageLoc = '#application.urls.root#logout.htm';
glJFAS.sLoggingOutLoc = '#application.urls.root#error/logoutwithmessage.htm';
	...
	if (form.quickaapp.value !== '' && form.quickaapp.value.length > 2 ) {
		...
		$.ajax(
			...
			, success: function(responseTxt,statusTxt,xhr){
				...
				else if (responseTxt.substr(0, 6) === 'Logout') {
					document.location = glJFAS.sLogoutNoMessageLoc;
					return false;
				}
...
/* comment out the new way
					// set the JS global, for use by displayAAPPsAjax
					glJFAS.sFieldString = 'home_filterSearchWord='+form.quickaapp.value+'&cboAgreementTypeFilter=&cboContractStatusFilter=&cboStateFilter=all&home_ContractStartDate1=&home_ContractEndDate1=&home_ContractStartDate2=&home_ContractEndDate2=';
					displayAAPPsAjax('');
...
function popAAPPsCompletionFailed () {
	...
	sReturn = EmailScopesAjax('User reached timeout on home page list of AAPPs');
	alert('The query you are trying to run is taking too long to respond. Your homepage settings will be returned to their default values.');

	// go to Home Page
	tReturn = ResetMyFilter();
...
function openOneSubmenu(idSubMenu, user_ID, IsHomePage) {
	var idArray = ['idMyAAPPs','idMyFilters','idNewAAPP','idHomePageOptions','idAAPPPageOptions','idNameOneFilter'];
	...
	if ($(selector).length){
		...
		if (idSubMenu == 'idHomePageOptions') {
			...
			+ '<ul>'
			+ '</div>';
			// alert('appending SHTML');
			$(document.body).append(SHTML);
			// alert('after appending SHTML');
		}
		...
		// TextPad "
			...
			nLeft	= 1 * $Source.offset().left + 300 + nXAdjustment;
...
// this routine from http://stackoverflow.com/questions/1144783/replacing-all-occurrences-of-a-string-in-javascript
function replaceAll(charold, charnew, ssource) {
	return ssource.replace(new RegExp(charold, 'g'), charnew);
}
