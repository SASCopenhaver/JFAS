...
<form name="frmGraph" id="frmGraph">
    ...
    <div id="div_JSON">div_JSON TEST area</div>
...
var datumTimeline = [],
	//
	datumTotals   = {},
		datumTotalOPS    = {},	// "subset" of datumTotals
		datumTotalCRA 	 = {},	// "subset" of datumTotals
		datumTotalOPSCRA = {},	// "subset" of datumTotals
	//
	datumOPS = {},
	datumCRA = {};
...
$(document).ready(function(){
	...
	var jqXHR = $.ajax({
				 ...
				 .success (function(objAllotmentGraph_JSON, statusTxt, xhr){
					 ...
					 //TEST3: $('#div_JSON').html(objAllotmentGraph_JSON);
					//
					...
					//
					datumTotals = f_makeWorkableJSON(parsedJSON.spr_graphTotals);	//TEST: alert(JSON.stringify( datumTotals))
					for ( var t in datumTotals ){
						datumTotalOPS    = datumTotals[0];
						datumTotalCRA    = datumTotals[1];
						datumTotalOPSCRA = datumTotals[2];
					}// TEST: alert("OPS:\n"+JSON.stringify(datumTotalOPS)+
								//"\nCRA:"+JSON.stringify(datumTotalCRA)+
								//"\nOPS CRA:"+JSON.stringify(datumTotalOPSCRA) );
								//$('#div_JSON').html(JSON.stringify( datumTotalOPS));
					//
					datumOPS = f_makeWorkableJSON(parsedJSON.spr_graphOPS);	//TEST: $('#div_JSON').html(JSON.stringify( datumOPS));
...
//---------------------------------------------------------------------------------------------
//CODE SAMPLE:
					//datumTotals = parsedJSON.spr_graphTotals.DATA;	//TEST:	alert(datumTotals)
