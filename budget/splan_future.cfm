                ...
                <td valign="top" >
                    ...
                    <cfelse>
                    	<div id="divSPNextPY_#H_LEVEL#_#PATH#">
                                <!---txtSPNextPY_#H_LEVEL#_#PATH#<br />--->
                                <input	type="text"
        ...
        <cfelse>
            ...
            <cfset variables.StyleDisplayTXA = "style='display:none;'">
                ...
                <td valign="top" colspan="2">
                                <!---txaNote_#H_LEVEL#_#PATH#<br />--->
                                <textarea name="txaNote_#H_LEVEL#_#PATH#" 
                                		  id="txaNote_#H_LEVEL#_#PATH#" 
                                          data-autoresize
                                          rows="3" 
                                          cols="25" 
                                          maxlength="900"
...
$("document").ready(function() {
	...
	$("input[id^=txtSPNextPY]").blur(function(){
		f_onBlur(this.name);
		f_updRecord(this.name);	
	});
	$("textarea[id^=txaNote]").focus(function(){
		f_msgShowHide("CHANGES_NO");
	});
	$("textarea[id^=txaNote]").blur(function(){
		f_updRecord(this.name);
	});
	$("textarea[id^=txaNote]").keyup(function(){
		f_textCounter(this.name, 900);
	});
//	$("textarea[id^=txaNote]").keydown(function(){
//		f_textCounter(this.name, 900);
//	});
...
//--------------------------------------------------------------
//	// The following resizes textarea ONLY vertically:
...
//--------------------------------------------------------------
function f_textCounter(arg_CtrlName, arg_Limit)
{
	var v_StrLength = $('##'+arg_CtrlName).val().length;
	if (v_StrLength > arg_Limit)
	{
		alert('The maximum size of this field is ' + arg_Limit + ' characters.');
		$('##'+arg_CtrlName).val($('##'+arg_CtrlName).val().substring(0, arg_Limit));
	}
}
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
function  f_updRecord(arg_CtrlName)
{
	var v_Pos = arg_CtrlName.indexOf("_");
	var v_CtrlName_Txt = "txtSPNextPY"+arg_CtrlName.substr(v_Pos);
	var v_CtrlName_Txa = "txaNote"+arg_CtrlName.substr(v_Pos);
	
	var v_SplanCatID = $("##"+v_CtrlName_Txt).attr("alt"); // ID is stored in property "alt".
	
	var v_Amnt = $("##"+v_CtrlName_Txt).val();
		v_Amnt = f_RemoveCommas(v_Amnt);

	var v_Note = $("textarea##"+v_CtrlName_Txa).val();
	
	var v_Splan = v_SplanCatID+"_"+v_Amnt+"_"+v_Note+"^"; 
	
	var cfcLink = '<cfoutput>'+'#application.urlstart##cgi.http_host##application.paths.components#splan.cfc?isBackground=yes'+'</cfoutput>';
	//
	
	if (v_Amnt != 0 || v_Note !='' )
	{
			var arrayMethod = { method: 'f_saveFutureSplan'
							   ,argUserID:v_UserID
							   ,argSplan:v_Splan
							  }
...
function f_onClick(arg_CtrlName)
{
	...
	{
		case "btnSave":
					of_Submit("splan_future_controller.cfm?actionMode=saveFutureSplan");
					// ORIGINAL CODE:
