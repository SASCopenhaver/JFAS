<cfsilent>
<!---
page: rpt_footprint_transaction_ncfms.cfm

description: display summary report for footprint transaction info (from NCFMS)
...
Revisions:
2009-01-07	mstein	File created
--->
</cfsilent>
...
<cfinvoke component="#application.paths.components#reports" method="getRptTransaction_ncfms" formdata="#form#" returnvariable="rsTransaction" />
...
<cfif isDefined("url.aapp") and isDefined("url.trans")>
	<!--- this code is used when called from the Reports tab under a specific AAPP --->
		...
		<table valig=top width="742" border="0" cellpadding="0" cellspacing="0" class="form2DataTbl">
			<tr><th colspan="<cfif form.hidReportType eq "O">7<cfelse>8</cfif>"></th>
			...
			<tr>
				...
				<th valign=top scope="col" style="text-align:center" title="Doc Type">Doc<br />Type</th>
				<th valign=top scope="col" style="text-align:center" title="Doc FY">Doc<br />FY</th>
				<th valign=top scope="col" style="text-align:center" title="Doc Num">Doc<br />Num</th>
				<th valign=top scope="col" style="text-align:center" title="Account ID">Account ID</th>
				<th valign=top scope="col" style="text-align:center" title="Cost Center">Cost Center</th>
			 ...
			 <cfloop query="rstransaction">
				...
				<cfif old_date neq "" and new_date neq old_date and currentrow neq 1 >
					<tr <cfif change_color>
							...
							<td colspan="7"></td>
							<td colspan="<cfif form.hidReportType eq "O">1<cfelse>2</cfif>" nowrap>subtotal for #dateFormat(old_date, 'mm/dd/yyyy')#</td>
				...
				<tr valign="top"
					<cfif change_color> class="form2AltRow" </cfif> >
					<td scope="row"><cfif cnt eq 1>#dateFormat(trans_date, 'mm/dd/yyyy')#</cfif></td>
					<td>#doc_type#</td>
					...
					<td style="border-left:1px solid ##5e84a6;" align="right" nowrap><cfif fund_cat eq "ops">#numberFormat(amount, "$,")#<cfelse>0</cfif></td>
					<td style="border-right:1px solid ##5e84a6;" align="right" nowrap><cfif fund_cat eq "cra">#numberformat(amount, "$,")#<cfelse>0</cfif></td>
					<td></td>
					<td></td>
					<td></td>
				</tr>
	...
	<!---</cfdocument>--->
<cfelse>
	<!--- this code is used when calling the report from the JFAS Reports menu --->
	<body class="form">
	<div class="formContent">
	<cfoutput>
	  ...
	  <!--- display title --->
		...
		<!--- display data --->
 	    <table valig=top width="742" border="0" cellpadding="0" cellspacing="0" class="form2DataTbl">
			<tr><th colspan="<cfif form.hidReportType eq "O">8<cfelse>9</cfif>"></th>
				<th colspan="2" style="text-align:center">#typeDesc#</th>
				<th colspan="3" style="text-align:center">Cumulative to Date</th>
			</tr>
			<tr>
		 ...
		 <cfif rstransaction.recordcount gt 0>
			 <cfloop query="rstransaction">
				...
				<tr <cfif change_color>
					...
					<td valign=top scope="row"><cfif cnt eq 1>#dateFormat(trans_date, 'mm/dd/yyyy')#</cfif></td>
					<td>#doc_type#</td>
					<td>#right(doc_fy,2)#</td>
					<td>#doc_num#</td>
					<td>#account_id#</td>
					...
					<td valign=top style="border-left:1px solid ##5e84a6;" align="right" nowrap><cfif fund_cat eq "ops">#numberFormat(amount, "$,")#<cfelse>0</cfif></td>
					<td valign=top style="border-right:1px solid ##5e84a6;" align="right" nowrap><cfif fund_cat eq "cra">#numberformat(amount, "$,")#<cfelse>0</cfif></td>
					<td nowrap></td>
					<td nowrap></td>
					<td nowrap></td>
				</tr>
