...
<cfif not variables.error>
	...
	<cfif form.txtDateStart NEQ "" and form.txtDateEnd NEQ "">
		<cfset variables.whereClause = variables.whereClause & " and (dateUpdate between to_date('#form.txtDateStart#','mm/dd/yyyy') and to_date('#form.txtDateEnd#','mm/dd/yyyy'))"> 
	<cfelseif form.txtDateStart NEQ "">
		<cfset variables.whereClause = variables.whereClause & " and (dateUpdate >= to_date('#form.txtDateStart#','mm/dd/yyyy'))"> 
	<cfelseif form.txtDateEnd NEQ "">
		<cfset variables.whereClause = variables.whereClause & " and (dateUpdate <= to_date('#form.txtDateEnd#','mm/dd/yyyy'))"> 
	</cfif>
	
	<!--- User Name --->
	<cfif form.cboUserName NEQ "all">
		<cfset variables.whereClause = variables.whereClause & " and (updatedBy = '#form.cboUserName#')"> 
	</cfif>
	
	<!--- Record Status --->
	<cfif form.radXactnStatus NEQ "all">
		<cfset variables.whereClause = variables.whereClause & " and (xactnStatusID = #form.radXactnStatus#)"> 
	</cfif>
