...
<cffunction name="headerDisplayContractInfo">
	<cfif request.aapp neq 0>
		<h1>#BuildSummaryTitle( request.aapp, request.centerName, request.ProgramActivity)#</h1>
		<!--- comment out old way <h1>ORANGE AAPP #request.aapp# #request.centerName#</h1> --->
...
<cffunction name="BuildSummaryTitle">
	<cfargument name="aapp">
	<cfargument name="centerName">
	<cfargument name="ProgramActivity">

	<cfset var sRet = 'AAPP ' & arguments.aapp>

	<cfif arguments.centername NEQ ''>
		<cfset sRet = sRet & "  " & arguments.centerName & ":">
	</cfif>
	<cfif arguments.ProgramActivity NEQ ''>
		<cfset sRet = sRet & '  ' & arguments.ProgramActivity>
	</cfif>

	<cfreturn sRet />

</cffunction> <!--- BuildSummaryTitle --->
