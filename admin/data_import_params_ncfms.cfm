...
description: Edit NCFMS data import parameters

revisions:
2014-03-31	mstein	Page created
2014-07-21	mstein	Fixed cancel button
2015-02-23	mstein	Added input parameter (importType) to ahndle multiple import processes
...
<cfif isDefined("form.btnSave")> <!--- coming from form submittal --->
	...
	<cfinvoke component="#application.paths.components#import_data" method="saveDataImportParams" importType="#url.importType#" formData="#form#" >
	<cflocation url="#cgi.SCRIPT_NAME#?saved=1">
<cfelse>
	<cfinvoke component="#application.paths.components#import_data" method="getDataImportParams" importType="#url.importType#" returnvariable="rstDataImportParams">
...
<h2><cfoutput>Data Import Parameters: #form.importTypeDesc#</cfoutput></h2>
...
<cfif isDefined("url.saved")>
	...
	<!--- Start Form --->
	<form name="frmEditUser"  action="<cfoutput>#cgi.script_name#?importType=#url.importType#</cfoutput>" method="post" onSubmit="return validateForm(this);">
	...
	<div class="buttons">
		...
		<cfset request.nextTabIndex = request.nextTabIndex + 1>
		<input type="hidden" name="importTypeDesc" value="#form.inputTypeDesc#">
	</div>
	</cfoutput>
	</form>
