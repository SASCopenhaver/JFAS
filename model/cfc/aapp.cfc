...
2014-10-23	mstein	Updated TestforInactive business rules (see v2.14 specs) related to FOPs and Pending Payments
2015-04-09	mstein	Update snapshot of successor when dates change
...
<cfcomponent displayname="AAPP" hint="Component that contains all general contract queries and functions">
	...
	<cffunction name="aappRequest" access="public" returntype="void" hint="Function runs for every request in aapp folder">
		...
		<cfelse> <!--- existing aapp --->
			...
			<cfset request.programActivity = rstAAPPHeaderInfo.programActivity>
		...
		<cfif success> <!--- if validation passed --->
			...
			<cfif arguments.formData.hidAAPP eq 0> <!--- new AAPP --->
				...
				values (
					...
					<cfif arguments.formData.cboSplanCatID neq "">#arguments.formData.cboSplanCatID#<cfelse>null</cfif>,
			...
			<cfelse> <!--- existing aapp --->
				...
				set		center_id = <cfif arguments.formData.cboCenter neq "">#listgetat(arguments.formData.cboCenter,1,"__")#<cfelse>null</cfif>,
						...
						splan_cat_id = <cfif arguments.formData.cboSplanCatID neq "">#arguments.formData.cboSplanCatID#<cfelse>null</cfif>,
				...
				<cfif listFindNoCase("DC,GR",arguments.formData.cboAgreement)>
					...
					<cfif (isDefined("arguments.formData.txtDateStart") and (arguments.formData.txtDateStart neq arguments.formData.hidDateStart)) or
						...
						<cfif qryGetSuccessor.recordcount neq 0>
                            ...
                            <!--- update snapshot table for successor --->
                            <cfstoredproc procedure="JFAS_SNAPSHOT_PKG.sp_updAAPP_CONTRACT_SNAPSHOT" returncode="no">
                                <cfprocparam type="in" cfsqltype="cf_sql_numeric" variable="argAAPP_NUM" value="#qryGetSuccessor.succAAPPNum#" null="no">
                                <cfprocparam type="in" cfsqltype="cf_sql_varchar" variable="argUser_ID" value="#session.userid#" null="no">
                                <cfprocparam type="in" cfsqltype="cf_sql_varchar" variable="argUpdate_Notes" value="Modified from AAPP Setup (predecessor)" null="no">
                            </cfstoredproc>
	...
	<cffunction name="testForInactive" access="public" returntype="string" hint="Determines whether an AAPP can be made inactive or not">
		...
		<!--- B. Does the AAPP have non-zero FOP balance for the current PY? (all AAPP types) --->
			...
			select	sum(amount) as FOPbalance
			from	rstFOPs
			where	programYear = #request.py#
			</cfquery>
			<cfif qryCurrentPYFOPs.FOPbalance neq "" and qryCurrentPYFOPs.FOPbalance neq 0>
				<cfset lstMessages = listAppend(lstMessages, "This AAPP can not be made inactive because it has a non-zero FOP balance (#numberformat(qryCurrentPYFOPs.FOPbalance,"$9,999")#) for the current Program Year.","~")>
			</cfif>
		...
		<!--- E. Any footprints with money left (payments < obglig) from the last 5 years --->
			...
			<!--- where pending payments (oblig - payment) is greater than $1) --->
			<cfquery name="qryCheckPaymentFootprints" >
			select	count(footprint_id) as numRecs
			from	footprint_ncfms
			where	aapp_num = #arguments.aapp# and
					oblig > 0 and
					round(oblig - payment) > 1 and
					approp_exp_py >= #request.py#-5
			</cfquery>
			<cfif (qryCheckPaymentFootprints.numRecs gt 0)>
	...
	<cffunction name="reactivateAAPP" access="public" output="false" returntype="void" hint="reactivates inactive AAPP">
        ...
        <cfstoredproc procedure="JFAS_SNAPSHOT_PKG.sp_updAAPP_CONTRACT_SNAPSHOT" returncode="no">
			...
			<cfprocparam type="in" cfsqltype="cf_sql_varchar" variable="argUser_ID" value="#session.userid#" null="no">
            <cfprocparam type="in" cfsqltype="cf_sql_varchar" variable="argUpdate_Notes" value="Modified from AAPP Reactivation" null="no">
		</cfstoredproc>
