...
Test this page: http://devetareports.doleta.gov/cfdocs/grantee_prod/jfas/extensions/scheduledevents/ncfms_DataImport_foot.cfm?isbackground=true
...
<cfcomponent name="import_data_ncfms" displayname="Import of NCFMS Data into JFAS" hint="Import of NCFMS Data into JFAS">
	...
	<cffunction name="validateNCFMSFootLoad" returntype="struct" output="false" hint="Validates NCFMS Foot Load prior to import, returns validation messages.">
		...
		<cfif qryDetermineFundingOffice.recordCount gt 0>
        	<cfset lstWarningMessages = listAppend(lstWarningMessages,"WARNING: There are #qryDetermineFundingOffice.recordCount# document IDs for which the funding office can not be determined - will be left blank (#ValueList(qryDetermineFundingOffice.document_id)#).","~~")>
		</cfif>			
    ...
    <cffunction name="validateNCFMSBudgetLoad" returntype="struct" output="false" hint="Validates NCFMS Budget Load prior to import, returns validation messages.">
		...
		<cfif qryDetermineOJCAgency.numRecs gt 0>
			...
			<cfquery name="qryDeleteNonOJCAgency" datasource="#request.dsn#">
			delete
			from	BUDGET_NCFMS_LOAD
			where	agency_code	<> 26
			</cfquery>
		</cfif>
        
        <!--- fund cat can NOT be determined --->
        <!--- note: in FOOT import, this is a critical error. In budget execution, it is a warning --->
		<cfquery name="qryCheckforFundCat" datasource="#request.dsn#">
		select	distinct fund_code, program_code
		from	budget_ncfms_load
		order	by fund_code, program_code
		...
		<cfloop query="qryCheckforFundCat">
			<cfset newFundCat = this.getNCFMSFundCat(qryCheckforFundCat.program_code, qryCheckforFundCat.fund_code)>
			<cfif newFundCat eq 0>
				<cfset lstWarningMessages = listAppend(lstWarningMessages,"WARNING: Funding Category cannot be determined (Program Code: #program_code#, Fund Code: #fund_code#).","~~")>	
			</cfif>
		</cfloop>		
		<!--- WARNING LEVEL VALIDATION: End --->
		
		<!--- ERROR LEVEL VALIDATION: look for validation errors that will halt import --->		
		<!--- no critical validation checks at this time --->
		<!--- ERROR LEVEL VALIDATION: end --->        
    ...
    <cffunction name="NCFMSBudgetImport" returntype="struct" output="true" hint="Transforms, Imports, NCFMS Budget Records">
		<cfargument name="userID" required="false" default="sys">
		
		<cfset lstImportMessage = "">
        <cfset instertedAppropriation = 0>
        <cfset instertedAllotment = 0>
		<cfset success = 1>
		<cfset strResults = StructNew()>
		
        <cftransaction>
        
        <!--- clean out APPROPRIATION_NCFMS and ALLOTMENT_NCFMS --->
        <cfquery name="qryDeleteApprop">
        delete
        from	APPROPRIATION_NCFMS
        </cfquery>
         <cfquery name="qryDeleteAllot">
        delete
        from	ALLOTMENT_NCFMS
        </cfquery>
        
        
		<!--- select all data from BUDGET_NCFMS_LOAD, including placeholder columns --->
        <cfquery name="qryGetBudgetLoad">
       	select	budget_ncfms_load.*,
       			'' as myFundCat,
                '' as myAppropPY,
                '' as myFundingOfficeNum
		from    budget_ncfms_load
        </cfquery>
        
        <!--- loop through recordset from budget load, and set calculated columns --->
        
        <cfset i = 1>
        <cfloop query="qryGetBudgetLoad">
        	
			<!--- pull appropriation FY, expiration FY from fund code --->
            <cfset appropFY = get4digitYear(mid(fund_code, 5, 2))>
            <cfif isNumeric(mid(fund_code, 7, 2))>
                <cfset expireFY = get4digitYear(mid(fund_code, 7, 2))>
            <cfelse>
                <cfset expireFY = "9999">
            </cfif>
            
            <!--- calculate appropriation PY, expiration PY --->
            <cfset FYdiff = expireFY - appropFY>
            <cfswitch expression="#FYdiff#">
                <cfcase value="0,2">
                    <cfset tempAppropPY = appropFY - 1>		
                </cfcase>
                <cfdefaultcase>
                	 <cfset tempAppropPY = appropFY>
                </cfdefaultcase>
            </cfswitch>			
			<cfset Temp = QuerySetCell(qryGetBudgetLoad, "myAppropPY", tempAppropPY, i)>
        	
        	<!--- determine fund category --->
			<cfset tempFundCat = this.getNCFMSFundCat(program_code, fund_code)>
			<cfif tempFundCat neq 0>
            	<cfset Temp = QuerySetCell(qryGetBudgetLoad, "myFundCat", tempFundCat, i)>			
            </cfif>	            
        
        	<!--- set funding office --->
        	<!--- determine funding office number --->
			<cfset tempFudingOfficeNum = this.getFootprintFundingOffice(managing_unit,0)>
			<cfif tempFudingOfficeNum neq -1>
				<cfset Temp = QuerySetCell(qryGetBudgetLoad, "myFundingOfficeNum", tempFudingOfficeNum, i)>	
			</cfif>
            
            <cfset i = i + 1>
        </cfloop> <!--- finish looping through budget load data --->
        
 
        <!--- ROLL UP QUERIES: to get appropriation, allotment amounts --->
        
        <!--- determine correct PY quarter --->
        <cfinvoke component="#application.paths.components#utility" method="GetCurrentQuarterNum" quarter_type="P" returnvariable="myQtrNum">
		
        <!--- query existing recordset to get appropriation amount by PY, fund cat --->
		<cfquery name="qryAppropRollup" dbtype="query">
        select	myAppropPY, myFundCat, sum(appropriation_amount) as myAppropriation
        from	qryGetBudgetLoad
        where	myFundCat is not null
        group	by myAppropPY, myFundCat
        order	by myAppropPY, myFundCat     
        </cfquery>
		
		<!--- query existing recordset to roll up and get obligation amounts by funding office, PY, fund cat --->
        <cfquery name="qryAllotmentRollup" dbtype="query">
        select	myAppropPY, myFundingOfficeNum, myFundCat, sum(obligation_amount) as myObligation
        from	qryGetBudgetLoad
        where	myFundCat is not null and myFundingOfficeNum is not null
        group	by myAppropPY, myFundingOfficeNum, myFundCat
        order	by myAppropPY, myFundingOfficeNum, myFundCat    
        </cfquery>

		<!--- LOOP THROUGH ROLL-UPs: to insert records into appropriation and allotment tables --->
		<!--- loop through appropriation recordset and write to APPROPRIATION_NCFMS --->
        <cfloop query="qryAppropRollup">
        	
			<cfquery name="addAppropriation">
            insert into APPROPRIATION_NCFMS (
            	fund_cat, py, amount)
            values (
            	'#myFundCat#', #myAppropPY#, #myAppropriation#)            
            </cfquery>
            <cfset instertedAppropriation = instertedAppropriation + 1>
		</cfloop>
        <cfset lstImportMessage = listappend(lstImportMessage,"#instertedAppropriation# records inserted into NCFMS Appropriation table.","~~")>
		
        
        <!--- loop through alotment recordset and write to ALLOTMENT_NCFMS --->
         <cfloop query="qryAllotmentRollup">
        	
			<cfquery name="addAllotment">
            insert into ALLOTMENT_NCFMS (
            	fund_cat, funding_office_num, py, Q#myQtrNum#_amount)
            values (
            	'#myFundCat#', #myFundingOfficeNum#, #myAppropPY#, #myObligation#)            
            </cfquery>
            <cfset instertedAllotment = instertedAllotment + 1>
        </cfloop>
        <cfset lstImportMessage = listappend(lstImportMessage,"#instertedAllotment# records inserted into NCFMS Allotment table.","~~")>
