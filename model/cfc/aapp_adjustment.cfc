...
2015-02-10	mstein	Added capability to add/update spend plan detail ID for FOPs
2015-03-09	mstein	Added getAAPPSplanCat, getSpendPlanDetails functions to support FOP to spend plan association
--->
...
<cffunction name="getAdjustmentList" access="public" returntype="query" hint="Lists AAPP Adjustments for Estimated Costs tab">
	...
	<cfquery name="qryGetAdjustmentList_EC" >
...
<cffunction name="createInitialBudgetAdjustments" access="public" output="true" hint="Creates Initial Budget Adjustments based on Contract Input">
	...
	<!--- TODO: add trigger--->
	<cfquery name="qryDeleteAdjustmentCosts" >
	delete
	from	adjustment_cost
	...
	<cfquery name="qryDeleteAdjustments" >
	delete
	from	adjustment
	where	aapp_num = #arguments.aapp# and
			adjustment_type_code in ('IBR','IBF','IBT')
	...
	<cfloop index="serviceType" list="#lstServiceTypes#">
		...
		<cfcase value="A,C1,C2"> <!--- if type is OPS or CTS --->
			<cfloop index="i" from="1" to="#aappLength#"> <!--- loop through contract years --->
                    ...
                    <cfquery name="qryAmountBaseFee" dbtype="query">
                        select	amount as amountBaseFee
                        from	rstContractInput
                        where	contract_year = #i# and
                                contract_type_code = '#serviceType#' and
                                budget_item_code in ('BF')
                    </cfquery> 

                    <cfquery name="qryAmountIncentiveFee" dbtype="query">
                        select	amount as amountInsentiveFee
                        from	rstContractInput
                        where	contract_year = #i# and
                                contract_type_code = '#serviceType#' and
                                budget_item_code in ('IF')
                    </cfquery>                    
                    
                    <cfif qryAmountBaseFee.amountBaseFee NEQ 0 AND qryAmountIncentiveFee.amountInsentiveFee EQ 0>
                    	<cfset vDescription = "Base Fee">
                    <cfelseif qryAmountBaseFee.amountBaseFee EQ 0 AND qryAmountIncentiveFee.amountInsentiveFee NEQ 0>
                    	<cfset vDescription = "Incentive Fee">
                    <cfelse>
                    	<cfset vDescription = "Base/Incentive Fee">
                    </cfif>
...
<cffunction name="getFOP" access="public" returntype="query" hint="Gets all data for an FOP record">
	...
	select	fop.fop_id as fopID,
			...
			adjustment_id as adjustmentID,
			splan_trans_det_id as splanDetail
	from	fop, adjustment
...
<cffunction name="getAAPPSplanCat" access="public" returntype="string" hint="Gets spend plan category for current AAPP">
	<cfargument name="aappNum" type="numeric" required="yes" />

	<cfquery name="qryGetAAPSplanCat" >
	select	splan_cat_id splanCatID
	from	aapp
	where	aapp_num = #arguments.aappNum#
	</cfquery>

	<cfreturn qryGetAAPSplanCat.splanCatID>
</cffunction> 


<cffunction name="getFOPSpendPlanDetails" access="public" returntype="query" hint="Gets list of spend plan transactions to display on FOP form">
	<cfargument name="py" type="numeric" required="yes">
    <cfargument name="splanSectionCodeList" type="string" required="yes">
    <cfargument name="FOPid" type="numeric" required="yes">

    
    <cfquery name="qryGetSpendPlanDetails">
    select  sp.splan_trans_id splantransid,
    		spdet.splan_trans_det_id splantransdetid,
            sp.trans_desc transdesc,
            splist.splan_cat_id splancatid,
            splist.cost_cat_id costcatid,
            spdet.amount, 
            sp.trans_status_code transstatuscode,
            lcc.cost_cat_code costcatcode
    from    splan_trans_det spdet,
            splan_trans sp,
            splan_cat_master_list splist,
            lu_cost_cat lcc
    where   spdet.splan_trans_id = sp.splan_trans_id and
            spdet.splan_cat_id = splist.splan_cat_id and
            splist.cost_cat_id = lcc.cost_cat_id (+) and
           	sp.py = #arguments.py# and            
            upper(splist.splan_section_code) in #ucase(application.outility.buildQuotedValueList(arguments.splansectionCodeList,"Alpha"))# and
            (sp.trans_status_code ='O' or (splan_trans_det_id in (select splan_trans_det_id from FOP where FOP_ID = #arguments.FOPid#)))
    order   by sp.splan_trans_id, amount
    </cfquery>
    
    <cfreturn qryGetSpendPlanDetails>
...
<cffunction name="saveAdjustmentFormData" access="public" returntype="struct" hint="Saves all Adjustment and FOP data (if applicable)">
	...
	<!--- spend plan detail may or may not exist --->
	<cfif isDefined("arguments.formData.txtSplanDetail")>
		<cfset splanDetail = arguments.formData.txtSplanDetail>
	<cfelse>
		<cfset splanDetail = "">
	</cfif>
...
<cffunction name="insertAdjustment" access="public" returntype="numeric" hint="Inserts record into adjustment table">
	...
	<cfquery name="qryGetNewID" >
	select	seq_adjustment.nextVal as newAdjustID
	from	dual
	</cfquery>

	<cfquery name="qryInsertAdjustment" >
...
<cffunction name="updateAdjustment" access="public" hint="Updates record in adjustment table">
	...
	<cfquery name="qryUpateAdjustment" >
...
<cffunction name="deleteAdjustment" access="public" hint="Deletes record from adjustment table">
	...
	<cfquery name="qryDeleteAdjustmentCost" >
	delete
	from	adjustment_cost
	where	adjustment_id = #arguments.adjustmentID#
	</cfquery>
	...
	<cfquery name="qryDeleteAdjustment" >
	delete
	from	adjustment
	where	adjustment_id = #arguments.adjustmentID#
	</cfquery>
...
<cffunction name="insertFOP" access="public" hint="Inserts record into FOP table">
	...
	<cfargument name="splanDetail" type="string" required="no" default="" />
	...
	values (
		...
		sysdate,
		'#arguments.backupLoc#',
		<cfif isDefined("arguments.splanDetail") and arguments.splanDetail neq "">#arguments.splanDetail#<cfelse>null</cfif>,
...
<cffunction name="updateFOP" access="public" hint="Updates record in FOP table">
	...
	update FOP set
		...
		splan_trans_det_id = <cfif isDefined("arguments.splanDetail") and arguments.splanDetail neq "">#arguments.splanDetail#<cfelse>null</cfif>,
...
<cffunction name="deleteFOP" access="public" returntype="void" hint="Deletes FOP record">
	...
	<cfquery name="qryDeleteFOPAssoc" >
	update	adjustment
	set fop_id = null
	where	fop_id = #arguments.fopID#
	</cfquery>

	<cfquery name="qryDeleteFOP" >
	delete
	from 	fop
	where	fop_id = #arguments.fopID#
	</cfquery>
...
<cffunction name="adjustOngoingEstimates" access="public" returntype="void" hint="Adjusts records in Adjustment Cost table based on change in contract length">
	...
	<cfif newContractLength lt oldContractLength>
		...
		<cfquery name="qryDeleteExtraAdjustmentCost" >
		delete
		from	adjustment_cost
		...
		<cfquery name="qryDeleteOrphanAdjustments" >
		delete
		from	adjustment
		where	aapp_num = #arguments.aapp# and
	...
	<cfelseif newContractLength gt oldContractLength>
		...
		<cfquery name="qryGetOngoingAdjustments" >
		...
		<cfswitch expression="#arguments.formData.hidActionType#">
			...
			<cfcase value="mov_diff">
				...
				<cfelse>
					...
					<cfquery name="qryUpdateAdjustmentAAPP" >
					update	fop
...
<cffunction name="getCumulativeFOPAmounts" access="public" returntype="query" hint="Returns query with cumulative FOPs through a given program year">
	...
	<cfquery name="qryCumulativeFOPAmounts" >
...
<cffunction name="getMinMaxAdjustmentDate" access="public" returntype="string" hint="Returns the earliest/latest adjustment effective date">
	...
	<cfquery name="qryGetMinMaxAdustmentDate" >
...
<cffunction name="getMinOptionYear" access="public" returntype="numeric" hint="Returns the minimum number of option years allowable (based on adjustments, mods">
	...
	<cfquery name="qryGetHighestModYear" >
	...
	<cfquery name="qryGetLatestEffectiveDate" >
	...
	<cfif qryGetLatestEffectiveDate.maxDate neq "">
		...
		<cfquery name="qryGetLatestEffectiveYear" >
...
<cffunction name="reverseProgramYearFOPs" access="public" returntype="void" hint="Reverses out all FOPs on an AAPP for a given PY">
	...
	<cfquery name="qryGetFOPTotalsbyCat" >
